...
_commander2.default.version(info.version).usage("<task> [name] [options]").option("-v, --version", "output the version number");
...
_commander2.default.command("preview <name>").description("preview a componout").action(function (name) {
	name = dashline(name);
	check(name);
	execute("cd " + cwd + " && " + gulp + " preview --name=" + name);
});

_commander2.default.command("test <name>").description("test a componout").action(function (name, options) {
	name = dashline(name);
	check(name);
...
_commander2.default.command("pull <name> [params...]").description("clone/pull a componout from https://github.com/componer").action(function (name, options, params) {
	name = dashline(name);
	check();
...
if (exists(current() + "/bin/custom-cli.js")) {
	require(current() + "/bin/custom-cli.js")(_commander2.default);
}
